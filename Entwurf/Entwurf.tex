...
Das Model-View-Template ist eine Ableitung des Architektur-Pattern-View-Presenter (MVP) -Musters und wird hauptsächlich zum Erstellen von Benutzerschnittstellen verwendet.\\
 ...
 \end{figure}
  \newpage
 	...
 	\begin{flushleft}
 	\textbf{•} Der \textbf{URL-Dispatcher} \emph{(urls.py)} ordnet die angeforderte URL einer \textbf{View-Funktion} zu und ruft sie auf.\\
 	...
 	\textbf{•} \textbf{Template} gibt normalerweise HTML-Seiten zurück. Die Django-Template-sprache bietet HTML-Autoren eine einfach zu erlernende Syntax und bietet gleichzeitig die gesamte für die Präsentationslogik erforderliche Leistung. \\
 	\end{flushleft}
 ...
 • Erstellt Django SQL-Tabellen aus der Modellen (klassen) mit alle klassen-Attributen als Tabellenfelder, z.b SQL-Tabelle "Request" aus dem Modell "Request". All dies geschieht wiederum ohne eine SQL-Abfrage zu schreiben.\\
...
\end{itemize} 
 ...
 Tabelle ``Resource`` speichert einen eindeutigen Identifikator zu jeder Ressource, ihren Namen, Typ, kurze Beschreibung und Erstellungsdatum.\\
  Tabelle ``ReceivedRequest`` ist die gesendeten Requests, ihre Empfänger, sowie ihren Typ zu speichern. Darum hat sie als Spalten Identifikator eines Benutzers und eines Requests und einen Typ des Requests
  
Eine Assoziation der Rechte wird in der Tabelle ``Permission`` gespeichert. Jeder Eintrag dieser Tabelle präsentiert entweder ein Zugriffs- oder Besitzerrecht, dabei werden folgende Attribute gespeichert: Identifikator des Benutzers, Identifikator der Ressource und Typ des Eintrags (Zugriffs- oder Besitzerrechte).
   \\
