...
\newpage
 \subsection{Paketenstruktur} %todo erweitern/korrigieren wenn möglich
 ...
 Design-Elemente, nötig für die HTML-Vorlagen, gehören zum Paket ``static``. In diesem Sinn benutzt ``templates`` das Paket ``static`` um die Vorlagen graphisch zu gestalten.\\
 Paket ``views`` enthält die so genannten Funktionen - Views, die als Ausgabe Aussicht der gewünschten Webadresse liefern. Diese Aussicht ist in der Form von einer HTML-Vorlage. Deshalb hängt ``views`` vom ``templates`` direkt ab.\\
 ``views`` braucht Informationen über den Datenbankzustand (z.B. Listen von Ressourcen darzustellen). Diese Informationen entnimmt ``views`` aus dem Paket ``models``. \\Das Paket ``views`` hat noch eine Abhängigkeit, nämlich von einem externen Paket mit zwei Unterpaketen - ``logging`` und ``eMailMessages``. ``logging`` ist notwendig um Ereignisse die eventuell während der Ausführung der View-Funktion passieren, zu protokollieren. ``eMailMessages`` unterstützt das Senden und Empfangen von E-Mail Benachrichtigungen. Dieses Paket definiert auch zahlreiche Vorlagen für die Nachrichten, die möglicherweise in einer View-Funktion benutzt werden.  \\
 Das Paket ``models`` ist auch dafür verantwortlich, das ``logging`` Modul über Änderungen in der Datenbank zu informieren, damit sie in der Log-Datei eingefügt werden. Das begründet die Nutzung von ``logging`` im Model.\\
 Die zwei Unterpakete - ``test{\_}views`` und ``test{\_}models`` benutzen entsprechend ``views`` und ``models``, um die Funktionen und Objekte von diesen Paketen zu testen. 
...
\begin{itemize}
	...
	Funktion liefert \inlinecode{List<User>} zurück, die alle Benutzer enthält, deren Daten (Name, Email, Domain) den \inlinecode{searchParameters} entsprechen.
	}
	
	\item \mtd{blockUser}{void}{userID:String}{public}{
	Nach Ausführung dieser Funktion wird der Benutzer, wem die   \inlinecode{userID} entspticht, sich nicht mehr ins Portal anmelden können.
	}
	\item \mtd{deleteUser}{void}{userID:String}{public}{
	Diese Funktion löscht die mit \inlinecode{userID} Einträge aus allen Tabellen aus der Datenbank: \inlinecode{user}, alle seine Rechte und Requests. Resourcen, für die er Rechte hat, werden nicht gelöscht.
	}
	\item \mtd{deleteResource}{void}{resourceID:String}{public}{
	Diese Funktion löscht die mit \inlinecode{resourceID} Einträge aus allen Tabellen aus der Datenbank: Metadaten von der Ressource, alle Rechte für sie und alle für sie erstellte Requests. Alle Benutzer, die Besitzerrechte für diese Ressource hatten, werden benachrichtigt.
	}
	\item \mtd{acceptDeletionRequest}{void}{requestID:String}{public}{
	\inlinecode{acceptRequest}, dem die \inlinecode{requestID} entspricht, wird genehmigt: Instanz der \inlinecode{Resource}-Klasse wird gelöscht, sowie alle entsprechende Einträge in der Datenbank: Metadaten von der Ressource, alle Rechte für sie und alle für sie erstellte Requests. Alle Benutzer, die Besitzerrechte für diese Ressource hatten, werden benachrichtigt.
	}
	...
	\item \mtd{deleteOwnerPermission}{void}{resourceID:String, userID:String}{public}{
	 Diese Funktion löscht den Eintrag mit \inlinecode{resourceID} und \inlinecode{userID} und Typ ``Òwner`` aus der Tabelle ``Permission`` gelöscht.
	}
\end{itemize}
\newpage
